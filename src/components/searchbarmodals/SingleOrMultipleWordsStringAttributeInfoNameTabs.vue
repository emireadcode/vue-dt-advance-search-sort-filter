  ...
  if(cards.value[index].concatenated !== undefined) {
    Object.values(cards.value[index].concatenated as MultipleWordsStringConcatenatedFieldType | SingleWordStringConcatenatedFieldType).forEach((concatenated, i) => {
      if(""+(bindex as number) === ""+i) {
        ...
        if(concatenated.enableincludeandexcludesearch !== undefined && concatenated.enableincludeandexcludesearch === true) {
          (concatenated.search as {includeorexcludestartswithcontainsendswithequaltoformat: 'STARTS-WITH' | 'CONTAINS' | 'ENDS-WITH' | 'EQUAL-TO';}).includeorexcludestartswithcontainsendswithequaltoformat = 'STARTS-WITH';
          (concatenated.search as {includeorexcludeformat: 'INCLUDE' | 'EXCLUDE' | '@NUMBER'}).includeorexcludeformat = 'INCLUDE';
        }
      }
      else {
...
function setTabDefaultSelection() {
  if(!cards.value[index].concatenated) {
    ...
    if(
      ...
      cards.value[index].search.multiple.includeorexcludestartswithcontainsendswithequaltoformat = 'STARTS-WITH';
...
function setConcatenatedTabClicked(cindex: number) {
  ((cards.value[index].concatenated as MultipleWordsStringConcatenatedFieldType | SingleWordStringConcatenatedFieldType)[cindex].search as {tabclicked: boolean}).tabclicked = true;
  triggerRef(cards);
      ...
      >
        ...
        <template v-else>
          <ul class="list-style-none flex-box flex-direction-row w-100 p-0 m-0 flex-nowrap justify-content-start align-items-center">
            ...
            >
              <button 
                :style="concatenated.search?.tabclicked? 'background-color:#F0E68C;' : 'background-color:lightgray;'"
