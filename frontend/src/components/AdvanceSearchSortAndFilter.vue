...
import { ref, shallowRef, onBeforeMount, type ShallowRef, provide } from 'vue'
import type {
  CardType,
  CardInnerType,
  DistinctRecordType,
...
onBeforeMount(() => {
  fetch('http://localhost:8080/distinct-record', {
    ...
    .then((res) => res.json())
    .then((json) => {
      cards.value = shallowRef(
        generateSearchSortAndFilterCards(props.config, json as DistinctRecordType)
      ) as ShallowRef<PrimitiveType[]>
    })
    .catch((_err) => {
      console.log(_err)
    })
