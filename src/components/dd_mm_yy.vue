...
import type { VisibleCalendarPropType, PositionTrackerType, DateSelectionsType, VisibleCalendarType, YearMonthClickable } from "./dd_mm_yy_types.vue";
import { resetYearMonthDayCalendarHolder, buildCalendar } from "./dd_mm_yy_utility_fns.vue";
...
const emits = defineEmits<{
  (e: "update:excludedatefor", action: "RANGE" | "MULTIPLE-OR-SINGLE"): void;
  (e: "enable:dd_mm_yyyy_excludecanceldoneforsearchreadiness", action: { mode: 'RANGE' | 'MULTIPLE-OR-SINGLE'; score: number; }): void;
}>();
...
let visiblecalendar = shallowRef<VisibleCalendarType>(),
  ...
  unwatchexcludedatefor: WatchStopHandle;
...
function updateFormat(fmt: 'RANGE' | 'MULTIPLE-OR-SINGLE') {
  ...
  nextTick(() => {
    emits('update:excludedatefor', fmt);
  });
}
...
onMounted(() => {
  unwatchexcludedatefor = watch(
    () => propsFromParent.excludedatefor,
    (x) => {
      if(x === "DISABLE") {
        if(Object.keys((visiblecalendar.value as VisibleCalendarType).selections).length > 0) {
          let result = findMaxAndMinDate(visiblecalendar.value as VisibleCalendarType);
          let selections = resetSelections((visiblecalendar.value as VisibleCalendarType).selections, result.maxdate, result.mindate, propsFromParent.isoweek);
          
          visiblecalendar.value = fillVisibleCalendarArray(
            result.maxdate, 
            result.mindate,
            {
              excludedatefor: propsFromParent.excludedatefor,
              maxdate: result.maxdate,
              mindate: result.mindate,
              isoweek: propsFromParent.isoweek,
              format: propsFromParent.format,
              title: propsFromParent.title,
              selections: selections,
            } as VisibleCalendarPropType
          );
          visiblecalendar.value.selections = selections;
          triggerRef(visiblecalendar);

          nextTick(() => {
            console.log(visiblecalendar);
            cupdatevcalendarvalue.value = true;
          });
        }
        else {
          ...
          let selections = resetSelections((visiblecalendar.value as VisibleCalendarType).selections, propsFromParent.maxdate, propsFromParent.mindate, propsFromParent.isoweek);
          visiblecalendar.value.selections = selections;
        ...
        if(x === "ENABLE") {
          let selections = (visiblecalendar.value as VisibleCalendarType).selections;
          cupdatevcalendarvalue.value = false;
          visiblecalendar.value = fillVisibleCalendarArray(propsFromParent.maxdate, propsFromParent.mindate, propsFromParent);
          
          if(Object.keys(selections).length > 0) {
            selections = resetSelections(selections, propsFromParent.maxdate, propsFromParent.mindate, propsFromParent.isoweek);
            visiblecalendar.value.selections = selections;
          }
...
<template>
  <div class="d-block" style="padding: 0 10px;">
    <PasteDD_MM_YY
      ...
      @send:dd_mm_yyyy_excludecanceldoneforsearchreadiness="$val => emits('enable:dd_mm_yyyy_excludecanceldoneforsearchreadiness', $val)"
    ...
    <div class="d-block" style="padding-bottom:2px;">
      <template v-if="propsFromParent.excludedatefor === 'DISABLE'">
        <VisibleCalendarDatePicker
          :vcalendar="(visiblecalendar as VisibleCalendarType)"
          :cformat="(clickformat as 'RANGE' | 'MULTIPLE-OR-SINGLE')"
          :excludemode="(true as boolean)"
