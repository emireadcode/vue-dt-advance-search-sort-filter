...
const fnc2 = computed(() => {
  return (props.from === 'NUMBER-SEARCHER-MODAL'
    ...
    : props.from === 'NUMBER-SEARCHER-MODAL'
    ? (holder.value as NumberType['search']['multiple'])
    : (holder.value as AtNumber<NumberSearchType>).search?.tab === 'GREATER-THAN'
    ? (holder.value as AtNumber<NumberSearchType>).search?.greaterthan
    : (holder.value as AtNumber<NumberSearchType>).search?.fromto?.from
})
